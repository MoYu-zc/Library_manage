...
import my_excel.People_Purchase_excel;
import my_excel.Purchase_history_excel;

public class People_purchase_watch_View extends JPanel implements ActionListener {
	...
	public People_purchase_watch_View(){
		this.setLayout(null);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==Search){
			...
			try {
				...
				String sql="select *from t_purchase where companyname="+"'"+search.getText()+"'";
		...
		if(e.getSource()==output){
			try {
				People_Purchase_excel.main_export();
				JOptionPane.showMessageDialog(this, "采购商信息表已导出至D盘！");
			} catch (Exception e1) {
				// TODO 自动生成的 catch 块
				e1.printStackTrace();
			}
