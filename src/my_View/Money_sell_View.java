...
import my_excel.Money_Sell_excel;
import my_excel.Purchase_history_excel;

public class Money_sell_View extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==Search){
			...
			try {
				...
				String sql="select *from t_sellbook where no_sell="+"'"+search.getText()+"'";
				...
				while(rs.next()){
					...
					for(int i=0;i<n&&rs.next();i++){
						...
						ob[i][7]=rs.getString(12);
					}
					String s[]={"销售单编号","图书ISBN","图书名称","单价","数量","销售时间","总额","经办人"};
		...
		if(e.getSource()==output){
			try {
				Money_Sell_excel.main_export();
				JOptionPane.showMessageDialog(this, "销售账款表已导出至D盘！");
			} catch (Exception e1) {
				// TODO 自动生成的 catch 块
				e1.printStackTrace();
			}
